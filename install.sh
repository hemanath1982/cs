...
print_usage() {
    cat <<EOF
Script to install and set-up CrowdStrike Falcon sensor.
This script shall be issued automatically with no arguments required.
Falcon credentials are needed to download falcon sensor. These may be supplied by
the way of env variables. Following variables may be used.
    - FALCON_CID
    - FALCON_CLIENT_ID
